...
const Register = () => {
    ...
    const handleSocialLogin = async (provider) => {
        ...
        setLoading(true);
        try {
            const usernameVal = `social_${provider.toLowerCase()}_user`;
            const emailVal = `${usernameVal}@example.com`;
                ...
                <div className="hidden lg:flex w-1/2 flex-col justify-between p-16 relative z-10">
                    ...
                    >
                        ...
                        <p className="text-slate-650 dark:text-slate-400 text-lg leading-relaxed max-w-md font-medium">
                            Create an account to track your progress, build ATS-friendly resumes, practice coding, and learn from top tutorials.
                        </p>
                    ...
                    >
                        ...
                        <div className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-2xl border border-slate-200 dark:border-slate-700/50 rounded-3xl p-8 shadow-xl dark:shadow-2xl shadow-slate-200/50 dark:shadow-black/40">
                            ...
                            <div className="mb-6">
                                ...
                                <p className="text-slate-655 dark:text-slate-400 text-sm mt-1.5 font-medium">Join the next-generation career ecosystem</p>
                            </div>
                            ...
                            {error && (
                                <motion.div
                                    ...
                                    animate={{ opacity: 1, y: 0 }}
                                    className="bg-rose-500/10 border border-rose-500/30 text-rose-700 dark:text-rose-305 text-xs rounded-xl p-3.5 mb-5 font-semibold flex items-start gap-2"
                                >
                            ...
                            {success && (
                                <motion.div
                                    initial={{ opacity: 0, y: -8 }}
                                    animate={{ opacity: 1, y: 0 }}
                                    className="bg-emerald-500/10 border border-emerald-500/30 text-emerald-700 dark:text-emerald-305 text-xs rounded-xl p-3.5 mb-5 font-semibold flex items-start gap-2"
                                >
                                    <span className="text-emerald-555 mt-0.5">✓</span> {success}
                                </motion.div>
                            )}
                            ...
                            <form onSubmit={handleSubmit} className="space-y-4">
                                ...
                                <div>
                                    ...
                                    <div className={`relative transition-all duration-200 ${focused === 'email' ? 'ring-2 ring-sky-500/30 rounded-xl' : ''}`}>
                                        <span className={`absolute inset-y-0 left-0 pl-3.5 flex items-center transition-colors ${focused === 'email' ? 'text-sky-550' : 'text-slate-450 dark:text-slate-550'}`}>
                                            <Mail size={16} />
                                        </span>
                                        <input
                                            type="email"
                                ...
                                <div>
                                    ...
                                    <select
                                        ...
                                        className="w-full bg-slate-100/50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700/60 rounded-xl py-2.5 px-4 text-sm text-slate-905 dark:text-slate-100 focus:outline-none focus:border-sky-500 transition-colors"
                                    >
                                        <option value="STUDENT" className="bg-white dark:bg-slate-800 text-slate-900 dark:text-white">Student / Learner</option>
                                        <option value="MENTOR" className="bg-white dark:bg-slate-800 text-slate-900 dark:text-white">Placement Mentor</option>
                                        <option value="ADMIN" className="bg-white dark:bg-slate-800 text-slate-900 dark:text-white">Admin Officer</option>
                                    </select>
                                </div>
                                ...
                                <motion.button
                                    ...
                                    className="w-full bg-gradient-to-r from-sky-500 to-indigo-600 hover:from-sky-400 hover:to-indigo-500 text-white rounded-xl py-3 text-sm font-bold shadow-lg shadow-sky-500/25 flex items-center justify-center gap-2.5 transition-all disabled:opacity-50 disabled:cursor-not-allowed mt-2"
                                >
                                    {loading ? (
            ...
            <div className="bg-slate-100/50 dark:bg-[#080B14] py-24 px-6 border-t border-slate-200 dark:border-slate-800/80 relative z-10 transition-colors duration-300">
                <div className="max-w-6xl mx-auto space-y-16">
                    ...
                    <div className="text-center space-y-4">
                        <h2 className="text-3xl md:text-5xl font-black bg-gradient-to-r from-sky-600 via-indigo-655 to-primary-600 dark:from-sky-400 dark:via-indigo-400 dark:to-primary-400 bg-clip-text text-transparent tracking-tight leading-tight">
                            Engineered for Placement Success
                        </h2>
                        <p className="text-slate-655 dark:text-slate-400 max-w-2xl mx-auto text-sm md:text-base font-semibold leading-relaxed">
                    ...
                    <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
                        ...
                        >
                            <div>
                                ...
                                <h3 className="text-slate-905 dark:text-white text-lg font-extrabold flex items-center justify-between">
                                    Coding Arena
                            ...
                            <div>
                                ...
                                {previewLoading ? (
                                    <div className="py-20 flex flex-col items-center justify-center gap-3">
                                        <Loader2 size={32} className="animate-spin text-sky-550 dark:text-sky-400" />
                                        <span className="text-slate-500 dark:text-slate-400 text-xs font-semibold">Fetching related database content...</span>
                                    </div>
                                ) : (
                                    <div className="space-y-4">
                                        <p className="text-slate-655 dark:text-slate-305 text-sm font-semibold mb-4">
                                            ...
                                            {previewData.length > 0 ? (
                                                previewData.map((item, index) => {
                                                    if (previewKey === 'coding') {
                                                        return (
                                                            <div key={item.id || index} className="p-5 bg-slate-100/50 dark:bg-slate-955/60 border border-slate-200 dark:border-slate-800 rounded-2xl flex flex-col justify-between gap-3 hover:border-primary-500/40 hover:bg-slate-200/60 dark:hover:bg-slate-905/60 transition-all group relative overflow-hidden">
                                                    ...
                                                    if (previewKey === 'learning') {
                                                        return (
                                                            <div key={item.id || index} className="p-5 bg-slate-100/50 dark:bg-slate-955/60 border border-slate-200 dark:border-slate-800 rounded-2xl flex flex-col justify-between gap-3 hover:border-emerald-500/40 hover:bg-slate-200/60 dark:hover:bg-slate-905/60 transition-all group relative overflow-hidden">
                                                    ...
                                                    if (previewKey === 'companies') {
                                                        return (
                                                            <div 
                                                                ...
                                                                className="p-5 bg-slate-100/50 dark:bg-slate-950/60 border border-slate-200 dark:border-slate-800 rounded-2xl flex flex-col justify-between gap-4 hover:border-amber-500/40 hover:bg-slate-200/60 dark:hover:bg-slate-900/60 hover:scale-[1.01] active:scale-[0.99] transition-all group relative overflow-hidden cursor-pointer"
                                                            >
                                                                ...
                                                                <div className="flex items-center justify-between border-t border-slate-200 dark:border-slate-800/80 pt-3">
                                                                    <div>
                                                                        ...
                                                                        <span className="text-xs font-black text-emerald-605 dark:text-emerald-400">
                                                ...
                                                <div className="py-12 text-center text-slate-550 dark:text-slate-500 text-xs font-semibold sm:col-span-2">
                    ...
                    <div className="w-full max-w-4xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl overflow-hidden shadow-2xl flex flex-col max-h-[90vh] text-left transition-colors duration-300">
                        ...
                        <div className="p-6 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between">
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    <div className="flex items-center gap-2">
                                        <h2 className="font-extrabold text-xl text-slate-900 dark:text-white">{selectedCompany.name}</h2>
                                        <span className="text-[10px] font-black uppercase tracking-wider text-sky-600 dark:text-sky-400 bg-sky-500/10 px-2 py-0.5 rounded border border-sky-500/20">
                                            {selectedCompany.industry || 'Technology'}
                                        </span>
                                    </div>
                                    <p className="text-xs text-slate-600 dark:text-slate-350 font-extrabold">
                            ...
                            <button 
                                ...
                                className="rounded-xl p-2 hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-500 dark:text-slate-400 hover:text-slate-905 dark:hover:text-white transition-colors"
                            >
                                <X size={20} />
                            </button>
                        </div>
                        ...
                        {/* Tabs Navigation */}
                        <div className="flex px-6 border-b border-slate-200 dark:border-slate-800 bg-slate-100/50 dark:bg-slate-950/20">
                            <button
                                onClick={() => setActiveTab('details')}
                                className={`px-4 py-3 text-xs font-black border-b-2 transition-all ${activeTab === 'details' ? 'border-primary-500 text-primary-600 dark:text-primary-400' : 'border-transparent text-slate-500 hover:text-slate-900 dark:text-slate-400 dark:hover:text-white'}`}
                            >
                                Eligibility & Rounds
                            </button>
                            <button
                                onClick={() => setActiveTab('jobs')}
                                className={`px-4 py-3 text-xs font-black border-b-2 transition-all ${activeTab === 'jobs' ? 'border-primary-500 text-primary-600 dark:text-primary-400' : 'border-transparent text-slate-500 hover:text-slate-900 dark:text-slate-400 dark:hover:text-white'}`}
                            >
                                Active Job Positions ({selectedCompany.jobPostsCount || 3})
                            </button>
                            <button
                                onClick={() => setActiveTab('prep')}
                                className={`px-4 py-3 text-xs font-black border-b-2 transition-all ${activeTab === 'prep' ? 'border-primary-500 text-primary-600 dark:text-primary-400' : 'border-transparent text-slate-500 hover:text-slate-900 dark:text-slate-400 dark:hover:text-white'}`}
                            >
                                Preparation Q&As
                            </button>
                        </div>
                        ...
                        <div className="p-6 md:p-8 overflow-y-auto flex-1 space-y-6 pb-12">
                            {activeTab === 'details' ? (() => {
                                ...
                                return (
                                    <div className="space-y-6">
                                        {/* Description Card */}
                                        <div className="p-5 bg-slate-100/50 dark:bg-slate-950/60 border border-slate-200 dark:border-slate-800/80 rounded-2xl space-y-2">
                                        ...
                                        {/* Stats Row */}
                                        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 bg-slate-100/50 dark:bg-slate-955/40 p-4 rounded-2xl border border-slate-200 dark:border-slate-800/80">
                                            <div>
                                                <span className="text-[10px] text-slate-550 dark:text-slate-400 font-bold block uppercase tracking-wider">Founded</span>
                                                <span className="text-sm font-black text-slate-905 dark:text-white mt-0.5">{selectedCompany.foundedYear || '1998'}</span>
                                            </div>
                                            <div>
                                                <span className="text-[10px] text-slate-550 dark:text-slate-400 font-bold block uppercase tracking-wider">Headquarters</span>
                                                <span className="text-sm font-black text-slate-905 dark:text-white mt-0.5">{selectedCompany.headquarters || 'USA'}</span>
                                            </div>
                                            <div>
                                                <span className="text-[10px] text-slate-550 dark:text-slate-400 font-bold block uppercase tracking-wider">Employee Count</span>
                                                <span className="text-sm font-black text-slate-905 dark:text-white mt-0.5">{selectedCompany.employeeCount || '10,000+'}</span>
                                            </div>
                                            <div>
                                                <span className="text-[10px] text-slate-550 dark:text-slate-400 font-bold block uppercase tracking-wider">Hiring Stage</span>
                                        ...
                                        <div className="grid gap-6 md:grid-cols-2">
                                            ...
                                            <div className="space-y-6">
                                                {/* Eligibility Box */}
                                                <div className="bg-slate-100/50 dark:bg-slate-955/60 border border-slate-200 dark:border-slate-800/80 rounded-2xl p-5 space-y-3">
                                                    <h4 className="text-xs font-black text-slate-555 dark:text-slate-400 uppercase tracking-wider flex items-center gap-1.5">
                                                        <BookOpen size={14} className="text-primary-500 dark:text-primary-400" /> Academic Eligibility
                                                    </h4>
                                                    <p className="text-xs font-bold text-slate-655 dark:text-slate-300 leading-relaxed bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-3.5 shadow-sm">
                                                        {selectedCompany.eligibility}
                                                    </p>
                                                </div>

                                                {/* Tech Stack Box */}
                                                <div className="bg-slate-100/50 dark:bg-slate-955/60 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 space-y-3">
                                                    <h4 className="text-xs font-black text-slate-555 dark:text-slate-400 uppercase tracking-wider flex items-center gap-1.5">
                                                ...
                                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                                    <div className="p-4 bg-slate-100/50 dark:bg-slate-955/60 border border-slate-200 dark:border-slate-800 rounded-2xl flex items-center gap-3">
                                                        ...
                                                        <div>
                                                            <span className="text-[10px] text-slate-550 dark:text-slate-400 font-bold uppercase tracking-wider block">Location</span>
                                                            <span className="text-xs font-black text-slate-800 dark:text-slate-200 leading-snug">{selectedCompany.jobLocation}</span>
                                                        </div>
                                                    </div>
                                                    <div className="p-4 bg-slate-100/50 dark:bg-slate-955/60 border border-slate-200 dark:border-slate-800 rounded-2xl flex items-center gap-3">
                                                        <div className="h-10 w-10 rounded-xl bg-rose-500/10 text-rose-600 dark:text-rose-500 flex items-center justify-center shrink-0 border border-rose-500/15">
                                                            <Calendar size={18} />
                                                        </div>
                                                        <div>
                                                            <span className="text-[10px] text-slate-555 dark:text-slate-400 font-bold uppercase tracking-wider block">Apply Before</span>
                                            ...
                                            <div className="space-y-6">
                                                {/* Timeline Box */}
                                                <div className="bg-slate-100/50 dark:bg-slate-955/60 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 space-y-4">
                                                    <h4 className="text-xs font-black text-slate-555 dark:text-slate-400 uppercase tracking-wider flex items-center gap-1.5">
                                                             ...
                                                             return (
                                                                <div key={i} className="relative group">
                                                                    ...
                                                                    <div className="p-3.5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl hover:border-primary-500/30 transition-all shadow-sm">
                                                                        <span className="text-xs font-extrabold text-slate-700 dark:text-slate-202 leading-relaxed block">{cleanRound}</span>
                                                                    </div>
                                                                </div>
                                                            );
                                                        })}
                                ...
                                <div className="space-y-4">
                                    {[
                                        ...
                                        <div key={job.code} className="bg-slate-105/50 dark:bg-slate-950/60 rounded-2xl p-5 border border-slate-200 dark:border-slate-800 space-y-3">
                                            ...
                                            <div className="flex flex-wrap gap-1.5 pt-1">
                                                {selectedCompany.requiredSkills.split(',').slice(0, 4).map((skill, i) => (
                                                    <span key={i} className="bg-slate-205 dark:bg-slate-800 text-[10px] font-black px-2.5 py-0.5 rounded text-slate-700 dark:text-slate-300">
                                                        {skill.trim()}
                                                    </span>
                                                ))}
                                            </div>
                                ...
                                <div className="space-y-4">
                                    {prepLoading ? (
                                        ...
                                        <div className="text-center py-12 bg-slate-105/50 dark:bg-slate-950/60 rounded-2xl border border-slate-200 dark:border-slate-800">
                                         ...
                                         prepQuestions.map((q) => (
                                             <div key={q.id} className="bg-slate-105/50 dark:bg-slate-955/60 rounded-2xl p-5 border border-slate-200 dark:border-slate-800 space-y-2.5">
                         ...
                         {/* Modal Footer */}
                         <div className="p-4 border-t border-slate-200 dark:border-slate-800 flex items-center justify-between bg-slate-100/50 dark:bg-slate-900/20 px-6">
